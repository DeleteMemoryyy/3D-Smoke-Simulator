...
float PerlinNoise2D(float fx, float fy, float alpha, float beta, int n)
{
	int ii;
	float val, sum = 0;
	float p[2], scale = 1;
...
C_SmokeSource::C_SmokeSource(int _id, vec3f _color, vec3f _pos, float _r, float _initialD, float _dD, bool _fixedDensity, float _initialVC, float _initialTC) : shape(Ball), id(_id), color(_color), pos(_pos), r(_r), initialD(_initialD), dD(_dD), fixedDensity(_fixedDensity), initialVC(_initialVC), initialTC(_initialTC)
{
}
C_SmokeSource:: ~C_SmokeSource() {}
...
C_RecSmokeSource::C_RecSmokeSource(int _id, vec3f _color, vec3f _pos, float _r, float _initialD, float _dD, bool _fixedDensity, float _initialVC, float _initialTC, float _w, float _h, float _d) : C_SmokeSource(_id, _color, _pos, _r, _initialD, _dD, _fixedDensity, _initialVC, _initialTC), w(_w), h(_w), d(_d)
{
	shape = Cuboid;
}
C_RecSmokeSource::~C_RecSmokeSource() {}

C_CFD::C_CFD() : NX(CELLS_IN_X), NY(CELLS_IN_Y), NZ(CELLS_IN_Z)
{
	p1x = (NX + 2) / 2;
	p0x = -p1x;
	p1y = (NY + 2) / 2;
...
int C_CFD::addSource(vec3f _color, vec3f _pos, SourceShape shape, float _r, float _initialD, float _dD, bool _fixedDensity, float _initialVC, float _initialTC, float _w, float _h, float _d)
{
...
vec3f C_CFD::getVorticity(int i, int j, int k)
{
	...
	return vec3f(DwDy - DvDz, DuDz - DwDx, DvDx - DuDy);
}

inline float nrm1(const vec3f &v)
{
	return (fabs(v.x) + fabs(v.y) + fabs(v.z));
}

inline float nrm2(const vec3f &v)
{
...
void C_CFD::render(int op, int frameN, vec3f canvasL, vec3f canvasH,
	GLuint *NoForAll, int textureCnt)
{
	...
	case 1:
		for (int a_s = 0; a_s < sourceNum; a_s++)
		{
			...
			int cnt = (tmpgs000 < gs_epslion) + (tmpgs001 < gs_epslion) + (tmpgs010 < gs_epslion) + (tmpgs100 < gs_epslion) + (tmpgs110 < gs_epslion) + (tmpgs101 < gs_epslion) + (tmpgs011 < gs_epslion) + (tmpgs111 < gs_epslion);
					...
					for (int kk = 0; kk < nz; kk++)
					{
						...
						if (op == 0)
						{
							...
							draw_box(tmpx + ii * dx, tmpx + ii * dx + dx,
								...
								source[a_s]->color.x, source[a_s]->color.y, source[a_s]->color.z, tmpgs,
								NoForAll, idx);
							delete[] idx;
						}
						else
...
void C_CFD::show_all_source(vec3f canvasL, vec3f canvasH)
{
	for (int a_s = 0; a_s < sourceNum; a_s++)
	{
		source[a_s]->draw_source(canvasL, canvasH);
	}
}

void C_SmokeSource::draw_source(vec3f canvasL, vec3f canvasH)
{
	float tmpx = (canvasH.x - canvasL.x) * (float)i / (float)CELLS_IN_X + canvasL.x;
	float tmpy = (canvasH.y - canvasL.y) * (float)j / (float)CELLS_IN_Y + canvasL.y;
	float tmpz = (canvasH.z - canvasL.z) * (float)k / (float)CELLS_IN_Z + canvasL.z;
	float dx = (canvasH.x - canvasL.x) / (float)CELLS_IN_X;
	float dy = (canvasH.y - canvasL.y) / (float)CELLS_IN_Y;
	float dz = (canvasH.z - canvasL.z) / (float)CELLS_IN_Z;

	vec3f oriPos(tmpx + (float)pos.x*dx + dx / 2.f, tmpy + (float)pos.y + dy / 2.f, tmpz + (float)pos.z + dz / 2.f);

	float surfaceAmbient[4] = { 0., 0., 0, 0. };
	float surfaceDiffuse[4] = { color.x, color.y, color.z, initialD / 2.f };
	...
	glColor4f(color.x, color.y, color.z, initialD / 2.f);

	draw_sphere(oriPos.x, oriPos.y, oriPos.z, r*dx, r*dy, r*dz, 100.f, 100.f);

}

void C_RecSmokeSource::draw_source(vec3f canvasL, vec3f canvasH)
{
	float tmpx = (canvasH.x - canvasL.x) * (float)i / (float)CELLS_IN_X + canvasL.x;
	float tmpy = (canvasH.y - canvasL.y) * (float)j / (float)CELLS_IN_Y + canvasL.y;
	float tmpz = (canvasH.z - canvasL.z) * (float)k / (float)CELLS_IN_Z + canvasL.z;
	...
	float sW = w * dx, sH = h * dy, sD = d * dz;

	vec3f recOriPos(tmpx + (float)pos.x*dx - sW / 2.f, tmpy + (float)pos.y*dy - sH / 2.f, tmpz + (float)pos.z*dz - sD / 2.f);
	float box_l = recOriPos.x, box_r = recOriPos.x + sW,
		box_d = recOriPos.y, box_u = recOriPos.y + sH,
		box_f = recOriPos.z, box_b = recOriPos.z + sD;
...
bool C_CFD::loadBuf(string filePath)
{
	return buf.loadFrame(filePath);
}

int C_CFD::bufFrameNum()
{
	return buf.frameNum;
}

void C_CFD::clearField()
{
	memset(fU0, 0, size * sizeof(float));
	memset(fV0, 0, size * sizeof(float));
	memset(fW0, 0, size * sizeof(float));
	...
	{
		memset(DC0[i], 0, size * sizeof(float));
		memset(DC1[i], 0, size * sizeof(float));
	}
}